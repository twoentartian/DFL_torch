...
def dataset_arithmetic_addition(*args, **kwargs):
    ...
    dataset_train = ArithmeticDataset.load_from_file(f"{base_dir}/train.txt", modulus, name=str(base_dir), train=True, tokenizer_path=f"{base_dir}/tokenizer.txt")
    dataset_test = ArithmeticDataset.load_from_file(f"{base_dir}/val.txt", modulus, name=str(base_dir), train=False, tokenizer_path=f"{base_dir}/tokenizer.txt")

    output = DatasetSetup(dataset_name, dataset_type, dataset_train, dataset_test, labels="non_classifier")
    return output

def dataset_arithmetic_cubepoly(*args, **kwargs):
    dataset_name = str(DatasetType.arithmetic_cubepoly.name)
    dataset_type = DatasetType.arithmetic_cubepoly

    matching_folders = []
    pattern = r'modulus(\d+)_cubepoly'
    regex = re.compile(pattern)
    for item in os.listdir(default_path_arithmetic):
        item_path = os.path.join(default_path_arithmetic, item)
        if os.path.isdir(item_path) and regex.match(item):
