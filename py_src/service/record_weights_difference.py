...
class ServiceWeightsDerivationRecorder(Service):
    def __init__(self, interval, nodes_to_record: List[int], l1_save_file_name="weights_derivation_l1.csv", l2_save_file_name="weights_derivation_l2.csv"):
        super().__init__()
        self.l1_save_file = None
        self.l2_save_file = None
        self.layer_order = None
    ...
    @staticmethod
    def get_service_name() -> str:
        return "weights_derivation_recorder"
    ...
    def trigger_without_runtime_parameters(self, tick, node_name_and_model_stat):
        ...
        for node_name, model_stat in node_name_and_model_stat.items():
            for layer_name in self.layer_order:
                layer_tensor = model_stat[layer_name]
                x0 = layer_tensor - layer_tensor.mean()
                l1_distance = x0.abs().mean().item() # Mean absolute deviation
                l2_distance = x0.pow(2).mean().sqrt() # Root mean square error
                l1_distances.append(f'{l1_distance:.4e}')
                l2_distances.append(f'{l2_distance:.4e}')
            l1_row = ",".join([str(tick), *l1_distances])
            self.l1_save_file[node_name].write(l1_row + "\n")
