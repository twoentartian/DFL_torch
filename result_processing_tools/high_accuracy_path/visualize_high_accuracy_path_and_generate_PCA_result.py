    ...
    for single_sub_folder in all_sub_folders:
        ...
        for layer_name in sample_model.keys():
            ...
            if layer_name not in layers_and_trajectory.keys():
                ...
                logger.info(f"create lmdb layer: {layer_name}, size={np_size}")
                layers_and_trajectory[layer_name] = np.zeros(np_size) # we should allocate all memory now
            ...
            logger.info(f"write lmdb layer: {layer_name}")
